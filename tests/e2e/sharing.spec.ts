...
test.describe("sharing", () => {
  test("a comparison link opens with the same work in another browser", async ({
    ...
    await expect(opened.getByTestId("scenario-picker")).toHaveValue("mixed-line");
  ...
  test("opening a link runs nothing until Run is pressed", async ({ page }) => {
    ...
    await expect(page.getByTestId("scenario-picker")).toHaveValue("relay");
...
test.describe("local saving", () => {
  ...
  test("names, renames, reopens and deletes a policy", async ({ page }) => {
    ...
    await expect(page.getByTestId("tab-policy")).toContainText("keeper.lua");

    await saved.getByTestId("saved-rename").click();
  ...
  test("keeps working with a notice when storage is unavailable", async ({ page }) => {
    ...
    await expect(page.getByTestId("saved-policy")).toHaveCount(0);
